...
export function sumAssistantUsage(ctx: ExtensionContext): FooterState["totals"] {
  let input = 0;
  let output = 0;
  let cost = 0;
...
export function getAccumulatedAgentMs(state: FooterState): number {
  const current = state.currentAgentStartMs ? Date.now() - state.currentAgentStartMs : 0;
  return state.totalAgentMs + current;
}

export function getAccumulatedApiMs(state: FooterState): number {
